...
> Define el formato de mensaje entre agentes, qué decide cada uno y la regla
> de escalado. Es la referencia canónica: si un prompt o un workflow
> contradice este documento, se corrige el prompt/workflow.
...
- Si ningún área cubre la consulta: el router responde `SIN_AGENTE`. El
  coordinador NUNCA responde el fondo del asunto por sí mismo.
...
Reglas: un solo bloque por respuesta; parámetros completos y terminados (lo
aprobado es lo que se ejecuta); ninguna clave fuera de catálogo.
...
- Qué campos son dudosos (los marca para verificación humana).
...
- Responder encargos de otras áreas (los deriva).

## 4. Regla de escalado — cuándo decir "no sé" en vez de actuar

El auxiliar DEVUELVE sin actuar, en este formato:
...
Todo estado terminal se registra con expediente completo (consulta, brief,
instrucción, aprobación, resultado o incidencia).
